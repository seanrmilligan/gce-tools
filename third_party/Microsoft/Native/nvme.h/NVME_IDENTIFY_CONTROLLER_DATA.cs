...
public readonly struct NVME_IDENTIFY_CONTROLLER_DATA
{
    ...
    private readonly Byte[] _sn4;
    
    /// <summary>
    /// Serial Number (SN)
    /// </summary>
    public Byte[] SN => new [] { _sn0, _sn1, _sn2, _sn3, }
        .Concat(_sn4)
        .ToArray();
    ...
    public readonly Byte[] FR;
    
    /// <summary>
    /// Recommended Arbitration Burst (RAB)
    /// </summary>
    [FieldOffset(72)]
    public readonly Byte RAB;
    ...
    /// </summary>
    public readonly Byte[] IEEE => new []{ _ieee0, _ieee1, _ieee2 };

    [FieldOffset(76)]
    public readonly CMIC CMIC;                     // byte 76.     O - Controller Multi-Path I/O and Namespace Sharing Capabilities (CMIC)

    [FieldOffset(77)]
    public readonly Byte MDTS;               // byte 77.     M - Maximum Data Transfer Size (MDTS)
    
    [FieldOffset(78)]
    public readonly UInt16 CNTLID;             // byte 78:79.   M - Controller ID (CNTLID)
    
    [FieldOffset(80)]
    public readonly UInt32 VER;                // byte 80:83.   M - Version (VER)
    
    [FieldOffset(84)]
    public readonly UInt32 RTD3R;              // byte 84:87.   M - RTD3 Resume Latency (RTD3R)
    
    [FieldOffset(88)]
    public readonly UInt32 RTD3E;              // byte 88:91.   M - RTD3 Entry Latency (RTD3E)
    ...
    [FieldOffset(258)]
    public readonly Byte ACL;                // byte 258.    M - Abort Command Limit (ACL)
    
    [FieldOffset(259)]
    public readonly Byte AERL;               // byte 259.    M - Asynchronous Event Request Limit (AERL)
    ...
    [FieldOffset(262)]
    public readonly Byte ELPE;               // byte 262.    M - Error Log Page Entries (ELPE)
    
    [FieldOffset(263)]
    public readonly Byte NPSS;               // byte 263.    M - Number of Power States Support (NPSS)
    ...
    [FieldOffset(266)]
    public readonly UInt16 WCTEMP;             // byte 266:267. M - Warning Composite Temperature Threshold (WCTEMP)
    
    [FieldOffset(268)]
    public readonly UInt16 CCTEMP;             // byte 268:269. M - Critical Composite Temperature Threshold (CCTEMP)
    
    [FieldOffset(270)]
    public readonly UInt16 MTFA;               // byte 270:271. O - Maximum Time for Firmware Activation (MTFA)
    
    [FieldOffset(272)]
    public readonly UInt32 HMPRE;              // byte 272:275. O - Host Memory Buffer Preferred Size (HMPRE)
    
    [FieldOffset(276)]
    public readonly UInt32 HMMIN;              // byte 276:279. O - Host Memory Buffer Minimum Size (HMMIN)
    
    [FieldOffset(280)]
    [MarshalAs(UnmanagedType.ByValArray, SizeConst = 16)]
    public readonly Byte[] TNVMCAP;        // byte 280:295. O - Total NVM Capacity (TNVMCAP)
    
    [FieldOffset(296)]
    [MarshalAs(UnmanagedType.ByValArray, SizeConst = 16)]
    public readonly Byte[] UNVMCAP;        // byte 296:311. O - Unallocated NVM Capacity (UNVMCAP)

    [FieldOffset(312)]
    public readonly RPMBS RPMBS;                    // byte 312:315. O - Replay Protected Memory Block Support (RPMBS)

    [FieldOffset(316)]
    public readonly UInt16 EDSTT;              // byte 316:317. O - Extended Device Self-test Time (EDSTT)
    
    [FieldOffset(318)]
    public readonly Byte DSTO;               // byte 318.     O - Device Self-test Options (DSTO)
    
    [FieldOffset(319)]
    public readonly Byte FWUG;               // byte 319.     M - Firmware Update Granularity (FWUG)
    
    [FieldOffset(320)]
    public readonly UInt16 KAS;                // byte 320:321  M - Keep Alive Support (KAS)
    
    [FieldOffset(322)]
    public readonly HCTMA HCTMA;                    // byte 322:323  O - Host Controlled Thermal Management Attributes (HCTMA)

    [FieldOffset(324)]
    public readonly UInt16 MNTMT;              // byte 324:325  O - Minimum Thermal Management Temperature (MNTMT)
    
    [FieldOffset(326)]
    public readonly UInt16 MXTMT;              // byte 326:327  O - Maximum Thermal Management Temperature (MXTMT)
    ...
    [FieldOffset(516)]
    public readonly UInt32 NN;                 // byte 516:519. M - Number of Namespaces (NN)
    ...
    [FieldOffset(526)]
    public readonly UInt16 AWUN;               // byte 526:527. M - Atomic Write Unit Normal (AWUN)
    
    [FieldOffset(528)]
    public readonly UInt16 AWUPF;              // byte 528:529. M - Atomic Write Unit Power Fail (AWUPF)
    ...
    [FieldOffset(532)]
    public readonly UInt16 ACWU;               // byte 532:533  O - Atomic Compare & Write Unit (ACWU)
    ...
    public readonly Byte[] VS;           // byte 3072 : 4095.
}